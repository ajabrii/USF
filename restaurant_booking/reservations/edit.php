...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Capacity check (exclude current reservation)
    if (empty($errors) && $slotId > 0) {
        ...
        if (!$slotData) {
            ...
            $stmtTotal = $db->prepare("SELECT COALESCE(SUM(guests),0) AS total_guests FROM reservations WHERE slot_id = ? AND date = ? AND status = 'CONFIRMED' AND id != ?");
            $stmtTotal->bind_param('isi', $slotId, $date, $id);
            $stmtTotal->execute();
    ...
    if (empty($errors)) {
        $stmtUpd = $db->prepare('UPDATE reservations SET slot_id = ?, date = ?, guests = ?, phone = ?, notes = ? WHERE id = ?');
        $stmtUpd->bind_param('isissi', $slotId, $date, $guests, $phone, $notes, $id);
        $stmtUpd->execute();
        $stmtUpd->close();

        flashSet('success', 'Réservation mise à jour avec succès.');
        header('Location: /restaurant_booking/reservations/index.php');
        exit;
    }
}
...
$formDate   = $_POST['date']    ?? $res['date'];
$formSlot   = (int)($_POST['slot_id'] ?? $res['slot_id']);
$formGuests = $_POST['guests']  ?? $res['guests'];
$formPhone  = $_POST['phone']   ?? $res['phone'];
$formNotes  = $_POST['notes']   ?? $res['notes'];
...
<div class="page-header">
  <h1>Modifier la réservation #<?= (int)$res['id'] ?></h1>
...
<div class="form-card">
  ...
  <form method="POST" id="mainForm" novalidate>
    ...
    <div class="form-group">
      ...
      <input type="date" id="date" name="date" required min="<?= date('Y-m-d') ?>" value="<?= h($formDate) ?>">
    </div>
    ...
    <div class="form-group">
      ...
      <select id="slot_id" name="slot_id" required>
        ...
        <option value="<?= (int)$s['id'] ?>" <?= ($formSlot === (int)$s['id']) ? 'selected' : '' ?>>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <input type="number" id="guests" name="guests" min="1" max="20" required value="<?= h($formGuests) ?>">
      </div>

      <div class="form-group">
        <label for="phone">Téléphone</label>
        <input type="tel" id="phone" name="phone" required value="<?= h($formPhone) ?>">
      </div>
    </div>

    <div class="form-group">
      <label for="notes">Notes (optionnel)</label>
      <textarea id="notes" name="notes"><?= h($formNotes) ?></textarea>
    </div>

    <div class="form-actions">
      <button type="submit" class="btn btn-primary">💾 Enregistrer</button>
      <a href="/restaurant_booking/reservations/index.php" class="btn btn-secondary">Annuler</a>
    </div>
  </form>
</div>
